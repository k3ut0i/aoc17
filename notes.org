...
#+begin_src perl :exports both :var infile='inputs/day7' :results output
  use lib 'lib';
  use Day7;
  open my $fh, '<', $infile or die "File error $infile: $!";
  my @lines;
  ...
  Day7::write_network_graphviz($network_root->[0], $dotfile);
  print "Root node is $network_root->[1]->[0]\n"
    if @{$network_root->[1]} == 1;
  `twopi -Tjpg -o $img $dotfile`
#+end_src

#+RESULTS:
: Root node is svugo
A Radial Visualization of this network
